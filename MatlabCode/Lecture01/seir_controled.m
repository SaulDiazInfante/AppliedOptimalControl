function y = seir_controled(b,d,c,e,g,a,S0,E0,I0,R0,A,T)
...
while(test < 0)  
    ...
    for i = 1:M
        ...
        m21 = b*(N(i)+h2*m14) - d*(S(i)+h2*m11) ...
            - c*(S(i)+h2*m11)*(I(i)+h2*m13) ...
            - 0.5*(u(i)+u(i+1))*(S(i)+h2*m11);
        m22 = c*(S(i)+h2*m11)*(I(i)+h2*m13) - (e+d)*(E(i)+h2*m12);
        m23 = e*(E(i)+h2*m12) - (g+a+d)*(I(i)+h2*m13);
        m24 = (b-d)*(N(i)+h2*m14) - a*(I(i)+h2*m13);
        
        m31 = b*(N(i)+h2*m24) - d*(S(i)+h2*m21) ...
            - c*(S(i)+h2*m21)*(I(i)+h2*m23) ...
            - 0.5*(u(i)+u(i+1))*(S(i)+h2*m21);
        m32 = c*(S(i)+h2*m21)*(I(i)+h2*m23) - (e+d)*(E(i)+h2*m22);
        m33 = e*(E(i)+h2*m22) - (g+a+d)*(I(i)+h2*m23);
        m34 = (b-d)*(N(i)+h2*m24) - a*(I(i)+h2*m23);
        
        m41 = b*(N(i)+h*m34) - d*(S(i)+h*m31) ...
            - c*(S(i)+h*m31)*(I(i)+h*m33) - u(i+1)*(S(i)+h*m31);
    ...
    for i = 1:M
        ...
        m13 = -A + (lambda1(j) - lambda2(j))*c*S(j) ...
            + lambda3(j)*(g+a+d) + lambda4(j)*a;
        m14 = -lambda1(j)*b - lambda4(j)*(b-d);
        
        m21 = (lambda1(j)-h2*m11)*(d ...
            + c*0.5*(I(j) + I(j-1)) ...
            + 0.5*(u(j) + u(j-1))) ...
            - c*(lambda2(j)-h2*m12)*0.5*(I(j) + I(j-1));
        m22 = (lambda2(j)-h2*m12)*(e + d) - (lambda3(j)-h2*m13)*e;
        m23 = -A + ((lambda1(j)-h2*m11) ...
            - (lambda2(j)-h2*m12))*c*0.5*(S(j) ...
            + S(j-1)) + (lambda3(j)-h2*m13)*(g+a+d) ...
            + (lambda4(j)-h2*m14)*a;
        m24 = -(lambda1(j)-h2*m11)*b - (lambda4(j)-h2*m14)*(b-d);
        
        m31 = (lambda1(j)-h2*m21)*(d ...
            + c*0.5*(I(j) + I(j-1)) ...
            + 0.5*(u(j) + u(j-1))) ...
            - c*(lambda2(j)-h2*m22)*0.5*(I(j) + I(j-1));
        m32 = (lambda2(j)-h2*m22)*(e + d) - (lambda3(j)-h2*m23)*e;
        m33 = -A + ((lambda1(j)-h2*m21) ...
            - (lambda2(j)-h2*m22))*c*0.5*(S(j) ...
            + S(j-1)) + (lambda3(j)-h2*m23)*(g+a+d) ...
            + (lambda4(j)-h2*m24)*a;
        m34 = -(lambda1(j)-h2*m21)*b - (lambda4(j)-h2*m24)*(b-d);
        
        m41 = (lambda1(j)-h*m31)*(d + c*I(j-1) + u(j-1)) ...
            - c*(lambda2(j)-h*m32)*I(j-1);
        m42 = (lambda2(j)-h*m32)*(e + d) - (lambda3(j)-h*m33)*e;
        m43 = -A + ((lambda1(j)-h*m31) - (lambda2(j)-h*m32))*c*S(j-1) ...
            + (lambda3(j)-h*m33)*(g+a+d) + (lambda4(j)-h*m34)*a;
    ...
    test = min(temp1, min(temp2, ...
        min(temp3, min(...
        temp4, min(temp5, min(temp6, min(temp7, min(temp8, temp9))))))));
end

for i=1:M
    m1 = g*I(i) - d*R(i) + u(i)*S(i);
    m2 = g*0.5*(I(i)+I(i+1)) ...
        - d*(R(i)+h2*m1) + 0.5*(u(i)+u(i+1))*0.5*(S(i)+S(i+1));
    m3 = g*0.5*(I(i)+I(i+1)) ...
        - d*(R(i)+h2*m2) + 0.5*(u(i)+u(i+1))*0.5*(S(i)+S(i+1));

...
for i=1:length(grd.Nu)
    current_grd.U{i} = linspace(grd.Un{i}.lo(end), ...
        grd.Un{i}.hi(end),grd.Nu{i}(end))';
end
...
for i=1:length(out0.C)
    ...
    if isfield(options,'gN') && length(options.gN)>= i
        if dpm_sizecmp(options.gN{i},zeros(get_size(current_grd)))
            ...
            error('DPM:Internal', ...
            ['options.gN{' num2str(i) '} has incorrect dimesions']);
        end
...
if ~options.UseLine && ~options.CalcLine && ~options.UseLevelSet
    for i=1:length(grd.Nx)
        eval(['dyn.Jo{1}(' repmat(':,',1,i-1) ...
               'current_grd.X{i} > grd.XN{i}.hi' ...
               repmat(',:',1, length(grd.Nx)-i) ') = options.MyInf;'])
        eval(['dyn.Jo{1}(' repmat(':,',1,i-1) ... 
            'current_grd.X{i} < grd.XN{i}.lo' ...
            repmat(',:',1,length(grd.Nx)-i) ') = options.MyInf;'])
    end
end
